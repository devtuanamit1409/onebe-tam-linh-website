...
const Page: React.FC = (params: any) => {
  ...
  const fetchDataTinTuc = async () => {
    try {
      setLoading(true);
      const endpoint = `${process.env.URL_API}/api/bai-viets?${searchParams}&locale=${locale}&filters[type][$containsi]=Thông tin quan trọng`;
  ...
  const fetchDataTinTucWithFilter = async (
    ...
    try {
      ...
      const filters = [
        ...
        `filters[type][$containsi]=Thông tin quan trọng`,
          ...
          <div className="mobile:col-span-12 tablet:col-span-6 ">
            <TintucNoibat
              data={tintuc
                .filter(
                  (item: tintuc) =>
                    item.attributes.type === "Thông tin quan trọng"
                )
                .map((item: tintuc) => item.attributes)
