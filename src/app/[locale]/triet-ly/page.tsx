...
const searchData2 = {
  populate: ["main.seo.thumbnail", "main.danh_muc_bai_viets "].toString(),
};
...
const searchParams2 = new URLSearchParams(searchData2).toString();

export async function generateMetadata(params: any): Promise<Metadata> {
  const dataVeChungToi = await fetchData(
    `${ENDPOINT.GET_SANPHAM}?${searchParams2}}&locale=${params.params.locale}`
  );
  ...
  return {
    ...
    alternates: {
      canonical: "https://ntse.vn/dich-vu",
    },
    openGraph: {
      title:
...
const page = async (params: any) => {
  ...
  const dataTinTuc = await fetchData(
    `${ENDPOINT.GET_BAIVIET}?${searchParams}&locale=${locale}&filters[type][$eq]=Tin tức&pagination[page]=1`
  );

  const dataDichVu = await fetchData(
    `${ENDPOINT.GET_SANPHAM}?${searchParamsDichVu}&locale=${locale}`
  );
  const dichVu = (
    dataDichVu as {
      data: {
  ...
  const tintuc = baiViet.data.map((item) => item.attributes);

  const dataDanhMuc = await fetchData(
    `${ENDPOINT.GET_DANHMUCCON}?filters[category][$eqi]=Triết lý&locale=${locale}`
  );
  const danhMuc = (
    dataDanhMuc as {
      data: {
    ...
    <div>
      ...
      <div className="container">
        <div className=" flex-col justify-start items-center gap-6 flex mt-[40px]">
          <h1 className="text-black mobile:text-[32px] tablet:text-[40px] desktop:text-[54px] font-bold  capitalize leading-normal">
