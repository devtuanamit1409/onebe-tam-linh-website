...
const MegaMenu = ({
  ...
  // const findMenuItemWithSlug = async (slug: string) => {
  //   const menu = await data.find((item: any) => item.attributes.slug === slug);
  //   setDataMenu((prev: any) => [...prev, menu]);
  // };
  useEffect(() => {
    if (data) {
  ...
  // const megaMenuItem = [
  //   {
  //     key: findMenuItemWithSlug("san-pham").slug,
  //     title: findMenuItemWithSlug("san-pham").name,
  //     description: findMenuItemWithSlug("san-pham").description,
  //     url: `/${findMenuItemWithSlug("san-pham").slug}`,
  //     content: [
  //       {
  ...
  return (
    <>
      {activeKey === "doi-tac" || activeKey === "tin-tuc" ? null : (
        <div
          className={`hidden laptop:block px-[92px] py-[37.5px] border-t-2 border-[#28A645] absolute left-0 w-full bg-white z-40 transition-all duration-700 ease-in-out ${
            isMenuOpen
